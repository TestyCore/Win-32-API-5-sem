...
void SaveDataToRegistry(const std::vector<int>& numArr, const std::vector<Date>& dateArr);
void LoadDataFromRegistry(std::vector<int>& numArr, std::vector<Date>& dateArr);
...
LRESULT CALLBACK WindowProcedure(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
	...
	case WM_CREATE:
		AddMenus(hWnd);
		AddControls(hWnd);
		LoadDataFromRegistry(NumArr, DateArr);
		break;

	case WM_DESTROY:
		SaveDataToRegistry(NumArr, DateArr);
		PostQuitMessage(0);
		break;
...
void SaveDataToRegistry(const std::vector<int>& numArr, const std::vector<Date>& dateArr) {
	HKEY hKey;
	if (RegCreateKeyEx(HKEY_CURRENT_USER, L"Software\\Win32API", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, NULL) == ERROR_SUCCESS) {
		// Сохраните значения ваших векторов в реестре
		RegSetValueEx(hKey, L"NumArr", 0, REG_BINARY, reinterpret_cast<const BYTE*>(numArr.data()), numArr.size() * sizeof(int));
		RegSetValueEx(hKey, L"DateArr", 0, REG_BINARY, reinterpret_cast<const BYTE*>(dateArr.data()), dateArr.size() * sizeof(Date));
		RegCloseKey(hKey);
	}
}
...
void LoadDataFromRegistry(std::vector<int>& numArr, std::vector<Date>& dateArr) {
	HKEY hKey;
	if (RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Win32API", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
		DWORD dataSize = 0;
		if (RegQueryValueEx(hKey, L"NumArr", 0, NULL, NULL, &dataSize) == ERROR_SUCCESS) {
			numArr.resize(dataSize / sizeof(int));
			RegQueryValueEx(hKey, L"NumArr", 0, NULL, reinterpret_cast<BYTE*>(numArr.data()), &dataSize);
		}

		dataSize = 0;
		if (RegQueryValueEx(hKey, L"DateArr", 0, NULL, NULL, &dataSize) == ERROR_SUCCESS) {
			dateArr.resize(dataSize / sizeof(Date));
			RegQueryValueEx(hKey, L"DateArr", 0, NULL, reinterpret_cast<BYTE*>(dateArr.data()), &dataSize);
		}
